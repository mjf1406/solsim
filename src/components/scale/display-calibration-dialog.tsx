...
export function DisplayCalibrationDialog({
  ...
  const refMm = calibrationReferenceWidthMm(calibratorRef)
  ...
  if (open !== prevOpen) {
    setPrevOpen(open)
    if (open) {
      const mm = calibrationReferenceWidthMm(calibratorRef)
  ...
  useEffect(() => {
    ...
    const oldMm = calibrationReferenceWidthMm(prevCalibratorRef.current)
    const newMm = calibrationReferenceWidthMm(calibratorRef)
    prevCalibratorRef.current = calibratorRef
      ...
      >
        ...
        >
          <div className="relative min-w-0">
            ...
            >
              <div className="pointer-events-none flex h-12 w-full items-center justify-center overflow-hidden rounded-md">
                <img
                  src="/graphite-pencil.webp"
                  alt=""
                  className="max-h-12 w-full scale-x-[-1] object-contain object-left"
                />
              </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col items-stretch gap-4 py-2">
          <div
            className={cn(
              ...
              (calibratorRef === "pencil" || calibratorRef === "passport") &&
                "overflow-visible"
            )}
          >
            <div
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  calibratorRef === "pencil" ? "h-3 w-3" : "h-5 w-5"
                )}
                aria-hidden
              />
              <span
              ...
              <img
                src="/graphite-pencil.webp"
                alt=""
                className="pointer-events-none absolute top-0 left-0 h-full min-w-[220%] max-w-none scale-x-[-1] rounded-xl object-cover object-left shadow-sm"
                aria-hidden
              />
              )}
            </div>
          ...
          >
            ...
            <CollapsibleContent className="px-3 pt-0 pb-3">
              <ol className="list-decimal space-y-2 pl-4 text-left text-sm text-muted-foreground">
                <li>
                  Pick the same object you chose above (graphite pencil across
                  flats, ID-1 card, or passport long edge).
                </li>
                <li>
